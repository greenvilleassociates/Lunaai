...
export function BannerAd() {
  const [commercials] = useState<BannerAdData[]>(fallbackCommercials);
  ...
  useEffect(() => {
    if (isHovered || commercials.length === 0) return;
  ...
  return (
    ...
    >
      <Box sx={{ position: "absolute", inset: 0, backgroundImage: `url(${currentAd.imageUrl})`, backgroundSize: "cover", backgroundPosition: "center", filter: "brightness(0.4)", transition: "opacity 0.5s ease" }} />
      <Box sx={{ position: "absolute", inset: 0, background: currentAd.bgColor, opacity: 0.85, transition: "opacity 0.5s ease" }} />
      <Box sx={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: { xs: "20px 50px", lg: "20px 80px" }, height: "200px", textAlign: "center", color: currentAd.textColor }}>
        <h3 style={{ fontSize: "clamp(1rem, 3vw, 1.5rem)", fontWeight: 700, margin: 0, marginBottom: "6px", textShadow: "0 2px 8px rgba(0,0,0,0.3)" }}>
          {currentAd.title}
        </h3>
        <p style={{ fontSize: "clamp(0.75rem, 2vw, 0.95rem)", margin: 0, opacity: 0.95, textShadow: "0 1px 4px rgba(0,0,0,0.3)", maxWidth: "900px" }}>
          {currentAd.description}
        </p>
        <a href={currentAd.linkUrl} target="_blank" rel="noopener noreferrer"
          style={{ marginTop: "12px", padding: "6px 20px", background: "rgba(255,255,255,0.2)", backdropFilter: "blur(10px)", border: `2px solid ${currentAd.textColor}`, borderRadius: "24px", color: currentAd.textColor, textDecoration: "none", fontSize: "clamp(0.75rem, 2vw, 0.875rem)", fontWeight: 600, transition: "all 0.3s ease", cursor: "pointer" }}
          onMouseEnter={(e) => { e.currentTarget.style.background = currentAd.textColor; e.currentTarget.style.color = currentAd.bgColor.includes("gradient") ? "#1a1a1a" : currentAd.bgColor; }}
          onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.2)"; e.currentTarget.style.color = currentAd.textColor; }}
        >
          Learn More →
        </a>
      </Box>
      <IconButton onClick={handlePrevious} sx={{ position: "absolute", left: { xs: 8, lg: 16 }, top: "50%", transform: "translateY(-50%)", bgcolor: "rgba(255,255,255,0.2)", backdropFilter: "blur(10px)", color: currentAd.textColor, width: { xs: 32, lg: 40 }, height: { xs: 32, lg: 40 }, "&:hover": { bgcolor: "rgba(255,255,255,0.3)" } }}>
        <ChevronLeft sx={{ fontSize: { xs: 20, lg: 24 } }} />
      </IconButton>
      <IconButton onClick={handleNext} sx={{ position: "absolute", right: { xs: 8, lg: 16 }, top: "50%", transform: "translateY(-50%)", bgcolor: "rgba(255,255,255,0.2)", backdropFilter: "blur(10px)", color: currentAd.textColor, width: { xs: 32, lg: 40 }, height: { xs: 32, lg: 40 }, "&:hover": { bgcolor: "rgba(255,255,255,0.3)" } }}>
        <ChevronRight sx={{ fontSize: { xs: 20, lg: 24 } }} />
      </IconButton>
      <Box sx={{ position: "absolute", bottom: { xs: 8, lg: 12 }, left: "50%", transform: "translateX(-50%)", display: "flex", gap: { xs: 0.5, lg: 1 } }}>
        {commercials.map((_, index) => (
          <button key={index} onClick={() => setCurrentAdIndex(index)}
            style={{ width: index === currentAdIndex ? "24px" : "8px", height: "8px", borderRadius: "4px", border: "none", background: index === currentAdIndex ? currentAd.textColor : "rgba(255,255,255,0.4)", cursor: "pointer", transition: "all 0.3s ease" }}
